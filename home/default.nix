{ homeDirectory, nixpkgs, home-manager, ... }: {
  ...
  home.file.".spacemacs.env".text = ''
    ...
    LANG=en_GB.UTF-8
    LOGNAME=daniel
    PATH=${homeDirectory}/.nix-profile/bin:${homeDirectory}/.goenv/shims:${homeDirectory}/.rbenv/shims:${homeDirectory}/.pyenv/shims:${homeDirectory}/.nodenv/shims:${homeDirectory}/.cargo/bin:${homeDirectory}/go/bin:${homeDirectory}/.local/bin:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin
    PWD=${homeDirectory}
    SHELL=/bin/bash
    USER=daniel
    '';
  ...
  home.activation.cloneSpacemacs =
    home-manager.lib.hm.dag.entryAfter [ "linkGeneration" ] ''
      ...
      $DRY_RUN_CMD git checkout origin/develop
    '';
}
